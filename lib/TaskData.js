...
TaskData.toTaskData = function(taskName){
  ...
  taskData.cron.string = dataFile.cron;
  taskData.cron.iterator = cronParser.parseExpression(dataFile.cron);
  ...
  async.series([
      ...
      function(callback){
        ...
        f.read(pathDataFile, function(err, data){
          if(err){ 
            debug("err thrown"); 
            return callback(err);
          }
      ...
      function(callback){
        try{
          ...
          activityFile = require(pathActivityFile);
          return callback();
        } catch(err){
          return callback(err); 
        }
    ...
    debug("convert string to object:"+dataFile);
...
TaskData.prototype.setCron = function(freq){
  ...
  this.cron.string = freq;
  this.cron.iterator = cronParser.parseExpression(freq);
};
