...
var TaskConfigs = require('./TaskConfigs.js');
 ...
 * });
 */
TaskScheduler.prototype.start = function(options, cb){
  var self = this;

  if(!options){
    throw new Error("The 'options' object must be set!");
  }

  if(!options.tasksDir){
    throw new Error("The tasks dir must be set!");
  }


  if(runner.isEmpty()){
    var tasksDir = options.tasksDir;
    if(!f.exists(taskDir)){
      f.mkdir(taskDir);
    }

    debug('start() - Load stored tasks');
    manager.loadTasks(tasksDir, function(err, tasksSaved){
