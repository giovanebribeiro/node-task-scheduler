...
var events = require('events'),
    ...
    cluster = require('cluster'); // confirmar pra ver se precisa mesmo
    utile = require('utile');
...
TaskRunner.prototype.start = function(){
  ...
  that.on('event', function(type, pid, message){
   if(type === "task message"){
    // this moment, the task is over, and must be deleted from pool
    delete that.threads[message.taskName];
   } 
  });
