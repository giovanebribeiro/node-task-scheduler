      ...
      onExit = function(code, signal){
        ...
        switch(code){
          ...
          case 1: // negative delay, loop continues
            if(code === 0){
              self.emit('scheduler', 'task_message', this.pid, 'The task '+taskName+' executed successfully. Start the task again.');
            }else{
              self.emit('scheduler', 'task_message', this.pid, 'The task '+taskName+' have a negative delay. The task is not executed (implemented in child process) and loop continues.');
            }
          ...
          case 2: // next execution date bigger than end date. Loop ends
            ...
            break;
          default:
            self.emit('scheduler', 'task_error', this.pid, "Unknown exit code ("+code+").");
        }
      };
